...
const parser = new Parser({ operators: { logical: true, comparison: true, assignment: false, 'in': false } });
  ...
  for (const mapping of mappings) {
    try {
      ...
      switch (mapping.mapping_type) {
        case 'direct':
          value = values[mapping.source_address];
          break;
        case 'expression':
        case 'calculated':
          if (mapping.expression) {
            value = evaluateExpression(mapping.expression, values);
          }
          break;
        case 'lookup':
          if (mapping.source_address && mapping.lookup_table) {
            const rawVal = values[mapping.source_address];
            const table = typeof mapping.lookup_table === 'string'
              ? JSON.parse(mapping.lookup_table) : mapping.lookup_table;
            // lookup_table format: { "1": "RUNNING", "2": "IDLE", "3": "FAULT" }
            value = table[String(rawVal)] !== undefined ? table[String(rawVal)] : rawVal;
          }
          break;
      }
...
function evaluateExpression(expression, values) {
  ...
  for (const addr of matches) {
    ...
    addressMap[safeVar] = typeof values[addr] === 'boolean' ? (values[addr] ? 1 : 0) : (values[addr] || 0);
