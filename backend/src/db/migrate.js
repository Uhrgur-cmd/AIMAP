...
CREATE TABLE IF NOT EXISTS network_comments (
  ...
  comment TEXT,
  signals_referenced TEXT[], -- array of addresses referenced in this network
  logic TEXT, -- extracted expression/SCL from the network (best-effort)
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Standard data model definition (shared across all machines in a plant)
...
CREATE TABLE IF NOT EXISTS mappings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  machine_id UUID NOT NULL REFERENCES machines(id) ON DELETE CASCADE,
  target_signal VARCHAR(255) NOT NULL, -- references datamodel_signals.name
  mapping_type VARCHAR(50) NOT NULL CHECK (mapping_type IN ('direct', 'expression', 'lookup', 'calculated')),
  source_address VARCHAR(255), -- for direct mappings
  expression TEXT, -- for expression/calculated mappings
  lookup_table JSONB, -- for lookup mappings
  confidence REAL,
  validated_by_human BOOLEAN DEFAULT false,
  reasoning TEXT, -- AI explanation for the mapping suggestion
...
-- Insert default standard signals
