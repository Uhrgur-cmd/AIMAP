...
function parseTiaV17Plus(filePath, plcType, tiaVersion) {
  // TIA V17+ compiles program blocks into binary PEData.plf, which cannot be read directly.
  // However, the same archive may still contain PLC tag tables and some XML-based blocks.
  // We attempt a best-effort XML extraction before giving up.
  const result = {
    source: 'project_file',
  ...
  try {
    ...
    for (const entry of zip.getEntries()) {
      ...
      try {
        ...
        extractTiaFcFbInterfaces(parsed, result);
        extractTiaTagTables(parsed, result);
  ...
  if (result.blocks.length === 0) {
    throw new Error(
      `TIA Portal V17+ project archive detected.\n\n` +
      `Program blocks are stored in a proprietary binary format (PEData.plf) that cannot be parsed directly.\n\n` +
      `To import signals, export your blocks as XML from TIA Portal:\n\n` +
      `1. Open the project in TIA Portal\n` +
      `2. In the project tree, select: PLC_1 → Program blocks\n` +
      `3. Right-click → "Generate source from blocks"\n` +
      `   OR: Right-click → "Export" (TIA Portal V16+)\n` +
      `4. Select all blocks and export as XML\n` +
      `5. Zip all exported files into a single .zip archive\n` +
      `6. Upload the .zip file here\n\n` +
      `Tip: Also export PLC variable tables for additional signal context.\n\n` +
      `Alternative: Use the OPC UA Live Scan (machine must be reachable on the network)`
    );
  }

  for (const block of result.blocks) {
    if (block.db_number !== null) calculateAddresses(block);
  }
...
/**
 * Extract PLC tag tables (SW.Tags.PlcTagTable) from TIA Portal XML.
 * Tag tables contain I/O variables (%I, %Q, %M) and are stored as XML
 * even in TIA Portal V17+ projects alongside the binary PEData.plf.
 */
function extractTiaTagTables(parsed, result) {
  const tagTables = findAllDeep(parsed, 'SW.Tags.PlcTagTable');
  for (const table of tagTables) {
    const tableName = getNestedValue(table, 'AttributeList', 'Name') || 'PLC Tags';
    const block = { db_number: null, name: tableName, variables: [] };

    const tags = findAllDeep(table, 'SW.Tags.PlcTag');
    for (const tag of Array.isArray(tags) ? tags : [tags]) {
      if (!tag || typeof tag !== 'object') continue;
      const name = getNestedValue(tag, 'AttributeList', 'Name');
      const dataType = getNestedValue(tag, 'AttributeList', 'DataTypeName');
      const logicalAddress = getNestedValue(tag, 'AttributeList', 'LogicalAddress') || '';
      const comment = extractComment(tag);
      if (name && dataType) {
    ...
    if (block.variables.length > 0) result.blocks.push(block);
  }
}
...
function parseTiaProject(filePath, plcType) {
  ...
  // Calculate addresses for DB blocks (FC/FB variables already have symbolic addresses)
  for (const block of result.blocks) {
    if (block.db_number !== null) calculateAddresses(block);
  }
...
/**
 * Extract FC and FB interface sections (Input, Output, InOut) from TIA Portal XML.
 *
 * FC/FB code blocks are the "DBs that involve networks" — they contain both
 * program networks (handled by extractTiaNetworks) AND interface variable
 * declarations that are just as relevant for signal mapping.
 *
 * FB Static section is intentionally excluded here: those variables are
 * duplicated in the InstanceDB (SW.Blocks.InstanceDB), which is the
 * authoritative source for persistent FB data with correct byte addresses.
 *
 * Variables receive symbolic addresses (e.g. FC1.IN.MotorSpeed) so that
 * calculateAddresses() skips them (it only processes variables with empty addresses).
 */
function extractTiaFcFbInterfaces(parsed, result) {
  const blockDefs = [
    { xmlKey: 'SW.Blocks.FC', prefix: 'FC', sections: ['Input', 'Output', 'InOut', 'Return'] },
    { xmlKey: 'SW.Blocks.FB', prefix: 'FB', sections: ['Input', 'Output', 'InOut'] },
  ];

  for (const { xmlKey, prefix, sections } of blockDefs) {
    const fcfbBlocks = findAllDeep(parsed, xmlKey);
    for (const fcfb of fcfbBlocks) {
      const name = getNestedValue(fcfb, 'AttributeList', 'Name');
      const number = parseInt(getNestedValue(fcfb, 'AttributeList', 'Number') || '0');
      const blockLabel = name ? `${name}` : `${prefix}${number}`;
      ...
      for (const iface of ifaces) {
        ...
        for (const section of Array.isArray(secs) ? secs : [secs]) {
          ...
          for (const member of Array.isArray(members) ? members : [members]) {
            ...
            const varName = member['@_Name'];
            const datatype = member['@_Datatype'] || member['@_datatype'] || 'Unknown';
            const comment = extractComment(member);
            // Symbolic address: FC1.IN.MotorSpeed / FB1.OUT.Fault
            const sectionAbbr = sectionName.substring(0, 2).toUpperCase();
            block.variables.push({
              name: varName,
              address: `${prefix}${number}.${sectionAbbr}.${varName}`,
              type: normalizeDatatype(datatype),
              comment
            });
          }
...
/**
 * Decode MC7 bytecode from SUBBLK types 8/12/14 (OB/FC/FB compiled code).
 * Produces dependency chains showing which outputs depend on which inputs.
 * SCL-compiled blocks (all-zero MC7) are silently skipped.
 */
function parseStep7MC7Logic(dbfBuffer, dbtBuffer, result) {
  ...
  for (let i = 0; i < numRecords; i++) {
    ...
    const subblkTyp = parseInt(readField(dbfBuffer, recStart, fieldIndex.SUBBLKTYP) || '0');
    // 8=OB, 12=FC, 14=FB MC7 compiled code
    if (![8, 12, 14].includes(subblkTyp)) continue;
    ...
    // Skip SCL-compiled blocks (all zeros = no AWL/LAD bytecode)
    let allZero = true;
    ...
    try {
      ...
      for (const chain of chains) {
        ...
        const inputAddrs = chain.inputs.map(inp => inp.replace(/\s+/g, ''));
        const logicStr = chain.logic ? chain.logic.join('; ') : null;
...
function readDbtMemo(dbtBuffer, blockNum, expectedLen) {
  ...
  // When length is known (MC7 data): read contiguously after the 8-byte block header
  ...
  // Fallback: follow chain pointers (for text/AWL memos without known length)
  const chunks = [];
